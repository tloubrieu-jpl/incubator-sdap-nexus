    ...
    def calc(self, request, **args):
        ...
        for shortName in ds:
            ...
            results, meta = spark_driver(daysinrange, bounding_polygon,
                                         shortName, self._tile_service_factory, metrics_record.record_metrics, spark_nparts=spark_nparts,
                                         sc=self._sc)
...
def calc_average_on_day(nexus_tile_service_factory, metrics_callback, tile_in_spark):
    import shapely.wkt
    from datetime import datetime
    from pytz import timezone
    ISO_8601 = '%Y-%m-%dT%H:%M:%S%z'
    ...
    if len(timestamps) == 0:
        return []
    tile_service = nexus_tile_service_factory.get_service()
    ds1_nexus_tiles = \
